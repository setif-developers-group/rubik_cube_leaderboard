...
const Admin = () => {
  ...
  const generateQRCode = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error("Failed to send QR code. Please try again.");
      }
    } catch (error) {
  ...
  const clearAllRecords = async () => {
    try {
      // If user has entered email, include it for audit trail
      const requestBody = adminEmail.trim()
            ...
            <Card className="animate-slide-up" style={{
              ...
              <CardContent className="space-y-6 text-center">
                <div className="bg-white p-6 rounded-lg inline-block">
                  <img
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(JSON.stringify(qrCodeData))}`}
                    alt="Admin QR Code"
